...
本仓库提供 Windows Sandbox 测试环境，支持**两种测试模式**：

### 模式一：Mapped Mode（开发调试）
快速验证本地代码修改。将工作区映射进 Sandbox，直接运行本地 `bootstrap.ps1`。

```powershell
.\tests\sandbox\launch-sandbox.ps1 -Mode Mapped
```

### 模式二：Clean Clone Mode（生产验证）
模拟真实用户从 GitHub 克隆并执行。Sandbox 内全新环境，从 GitHub 克隆后运行。
...
**选择建议**：
- 开发调试 → **Mapped Mode**（快速，即时反馈）
- 发布前验证 → **Clean Clone Mode**（模拟真实用户）

详见 [`tests/sandbox/README.md`](tests/sandbox/README.md)。
...
```powershell
# 下载仓库
git clone https://github.com/gorvey-with-claw/win-bootstrap.git
cd win-bootstrap
...
> **注意**：当前 `bootstrap.ps1` 依赖仓库内的 `scripts/`、`apps.json`、`bucket/` 等文件，因此必须先下载完整仓库，再在仓库目录中执行。当前版本不支持直接远程执行单个 `bootstrap.ps1` 文件。
